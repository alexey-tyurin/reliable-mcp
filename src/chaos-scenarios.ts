...
import type { ReliableMcpTarget } from './chaos-config.js';
import type { FaultConfig } from 'mcp-chaos-monkey';

function fault(target: ReliableMcpTarget, config: FaultConfig, durationMs?: number): { target: ReliableMcpTarget; config: FaultConfig; durationMs?: number } {
  if (durationMs !== undefined) {
    return { target, config, durationMs };
  }
...
export const CHAOS_SCENARIOS = [
  defineScenario({
    ...
    faults: [
      fault('weather-api', { type: 'error', statusCode: 503 }),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('weather-api', { type: 'error', statusCode: 503 }, 35_000),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('flight-api', { type: 'timeout', hangMs: 10_000 }),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('redis', { type: 'connection-refused' }),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('redis-cache', { type: 'latency', delayMs: 2000 }),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('oauth-token', { type: 'error', statusCode: 401, message: 'Token expired' }),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('weather-mcp', { type: 'connection-refused' }),
      fault('flight-mcp', { type: 'connection-refused' }),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('weather-mcp', { type: 'malformed', corruptResponse: true }),
    ],
    expectedBehavior:
  ...
  defineScenario({
    ...
    faults: [
      fault('flight-api', { type: 'rate-limit', retryAfterSeconds: 60 }),
    ],
    expectedBehavior:
